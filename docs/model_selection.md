...
- `scripts/generate_data.py`는 10,000개 행을 결정적으로 생성합니다. 분류는 클래스 비율을 보존하는 80:20 분할을 사용하고, 회귀도 `random_state=42`로 동일한 분할 비율을 사용합니다.
- 특성 전처리는 각 scikit-learn `Pipeline` 내부에 있습니다. 따라서 결측치 대치와 스케일링은 교차검증 중에는 각 학습 폴드에서만 fit되고, holdout 추론 전에는 전체 학습 데이터에서만 fit됩니다.
- 랜덤 포레스트는 `n_estimators=100`을 고정하고, `max_depth` 후보(`None`, `8`, `16`)와 `min_samples_split` 후보(`5`, `10`, `20`, `40`, `80`)를 학습 데이터 내부 5-fold `GridSearchCV(scoring="roc_auc")`로 함께 비교합니다. 선택된 `max_depth`와 `min_samples_split`을 기준으로 이후 트리 수 민감도 분석의 비용·성능을 비교합니다. 보류 검증 세트의 레이블은 모델·하이퍼파라미터·임계값 선택에는 사용하지 않습니다. 보류 검증 세트는 선택이 완료된 모델과 정책의 최종 성능 평가 및 ROC 곡선, 혼동 행렬 같은 평가 시각화에만 사용합니다.
