...
- 랜덤 포레스트는 `n_estimators=100`을 고정하고, `max_depth` 후보(`None`, `8`, `16`)와 `min_samples_split` 후보(`5`, `10`, `20`, `40`, `80`)를 학습 데이터 내부 5-fold `GridSearchCV(scoring="roc_auc")`로 비교합니다. 초기 탐색에서 선택된 `max_depth=8`을 기준으로, 이후 트리 수 민감도 분석의 비용·성능을 비교합니다. 보류 검증 세트의 레이블은 모델·하이퍼파라미터·임계값 선택에는 사용하지 않습니다. 보류 검증 세트는 선택이 완료된 모델과 정책의 최종 성능 평가 및 ROC 곡선, 혼동 행렬 같은 평가 시각화에만 사용합니다.
