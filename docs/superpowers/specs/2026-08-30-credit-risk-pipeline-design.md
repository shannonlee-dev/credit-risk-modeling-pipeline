...
제공된 `data_gen.py` 로직으로 생성한 10,000건의 고객 데이터를 사용해 규칙 기반 분류, 머신러닝 분류, 규제 회귀, 앙상블 튜닝을 한 번의 재현 가능한 실행 흐름으로 제공한다. `mission.md`와 `rubric.md`의 필수 요구사항만 구현하며 Custom Transformer, 별도 패키지 모듈화, Learning Curve 같은 보너스 항목은 제외한다.
